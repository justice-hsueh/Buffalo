...
with col1:
    ...
    for ev in sorted(show_events, key=lambda x: x["日期"]):
        st.markdown(f'<div class="event-card show-style"><strong>📅 【{ev["日期"]}】<br>⏰ {ev["時間"]}</strong><br>📌 {ev["內容"]}</div>', unsafe_allow_html=True)
...
with col2:
    ...
    for ev in sorted(progress_events, key=lambda x: x["日期"]):
        st.markdown(f'<div class="event-card progress-style"><strong>📅 【{ev["日期"]}】<br>⏰ {ev["時間"]}</strong><br>📌 {ev["內容"]}</div>', unsafe_allow_html=True)
...
with col3:
    ...
    for ev in sorted(notice_events, key=lambda x: x["日期"]):
        st.markdown(f'<div class="event-card notice-style"><strong>📅 【{ev["日期"]}】<br>⏰ {ev["時間"]}</strong><br>📌 {ev["內容"]}</div>', unsafe_allow_html=True)
