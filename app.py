    ...
    if mode == "新增行程":
        with st.sidebar.form("add_f"):
            ...
            if st.form_submit_button("新增"):
                ...
                new_ev = {"分類": cat, "日期": f_date, "時間": time_in, "地點": loc_in, "內容": cont}
        ...
        # 顯示時確保能安全讀取地點
        opts = {f"{e['日期']} | {e.get('內容', '')[:10]}": e for e in st.session_state.events}
        ...
        to_del = st.sidebar.selectbox("選擇要刪除的行程", [f"{e['日期']} | {e.get('內容', '')[:10]}" for e in st.session_state.events])
        if st.sidebar.button("確認刪除"):
            st.session_state.events = [e for e in st.session_state.events if f"{e['日期']} | {e.get('內容', '')[:10]}" != to_del]
            with open("events.json", "w", encoding="utf-8") as f:
                json.dump(st.session_state.events, f, ensure_ascii=False, indent=4)
            st.rerun()
