...
import os
import base64
import Aimport streamlit as st
import pandas as pd
from datetime import datetime, date, timedelta
import os
import base64
...
with col3:
    ...
    for ev in sorted([e for e in st.session_state.events if e["分類"] == "📢 通知事項"], key=lambda x: get_sort_date(x["日期"])):
        lines = ev["內容"].split("\n")
        st.markdown(f'''<div class="event-card notice-style"><div class="title-text">📢 {lines[0]}</div>📅 {ev["日期"]}<hr>{render_content_items(lines[1:])}</div>''', unsafe_allow_html=True)son
